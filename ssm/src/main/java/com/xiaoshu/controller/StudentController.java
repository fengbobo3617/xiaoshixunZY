...
public class StudentController extends LogController{
	...
	public void userList(StudentVo studentVo,HttpServletRequest request,HttpServletResponse response,String offset,String limit) throws Exception{
		try {
			...
			PageInfo<StudentVo> userList= ss.findUserPage(studentVo,pageNum,pageSize,ordername,order);
	...
	public void reserveStudent(MultipartFile picFile,HttpServletRequest request,Student student,HttpServletResponse response) throws Exception{
		...
		if(picFile!=null && picFile.getSize()>0){
			...
			//设置虚拟路径
			picFile.transferTo(new File("D:/123photo/"+newName));
			//封装
		...
		try {
			if (sid != null) {   // userId不为空 说明是修改
				Student sname = ss.existUserWithUserName(student.getSname());
	...
	public void exportStudent(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			Log log = new Log();
			List<StudentVo> slist=ss.findStudent(new StudentVo() );
		...
		try {
			...
			for (int i = 0;i < list.size(); i++) {
			   ...
			   StudentVo studentVo = list.get(i);
			    ...
			    row.createCell(0).setCellValue(studentVo.getSid());
			    ...
			    row.createCell(4).setCellValue(studentVo.getGender());
			    row.createCell(5).setCellValue(studentVo.getScname());
			  
			}
			//写出文件（path为文件路径含文件名）
				OutputStream os;
				File file = new File("D:/123photo/"+"测试"+".xls");
